...
def rnn_step_backward(dnext_h, cache):
    ...
    dactivation = (1.0 - next_h ** 2) * dnext_h
...
def rnn_backward(dh, cache):
    ...
    h_wider = np.zeros((N, T + 1, H))
    h_wider[:, 1:, :] = h
    h_wider[:, 0, :] = h0
    dh_wider = np.zeros((N, T + 1, H))
    dh_wider[:, 1:, :] = dh
    for i in range(T-1, -1, -1):
        single_cache = (x[:, i, :], h_wider[:, i, :], Wh, Wx, b, h_wider[:, i + 1, :])
        temp_dx, temp_dprev_h, temp_dWx, temp_dWh, temp_db = rnn_step_backward(dh_wider[:, i + 1, :], single_cache)
        dx[:, i, :] = temp_dx
        dh_wider[:, i, ] += temp_dprev_h
        dWx += temp_dWx
        dWh += temp_dWh
        db += temp_db
    dh0 = dh_wider[:, 0, :]
